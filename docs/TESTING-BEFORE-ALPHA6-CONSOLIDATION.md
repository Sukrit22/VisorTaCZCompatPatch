# Consolidated testing checklist - 0.12.0-alpha.6

Included in alpha.6: [AH44–AH50 and cylinder instructions](PENDING-HANDLING-CHANGES.md). Use alpha.6 on client and server (protocol 22).

Start with [AH36–AH43: inspection, magazine motion, support transfer and pump retention](RELEASE-0.12.0-alpha.5.md). These are pending headset verification. Earlier outstanding checks are retained below.

New UI checks: [calibration Save/Close/Cancel and coarse holster steps](RELEASE-0.12.0-alpha.4.md).

Priority: [AH28â€“AH31: holster height and visible-handle grab alignment](RELEASE-0.12.0-alpha.3.md). Reset heights that were lowered to compensate for the alpha.2 bug. Headset retests are pending.

Start with [AH20â€“AH27: holsters, defaults, tosses and reconnect](RELEASE-0.12.0-alpha.2.md). All remain pending headset verification.

**New experimental checks:** [AH01â€“AH19 Advanced Handling](RELEASE-0.12.0-alpha.1.md). All are pending headset testing. Current protocol 22 requires matching client/server builds. Keep the previous outstanding tests below; no previous pass has been promoted to a pass for this alpha.
...
**Previous baseline:** 0.11.0, protocol 19. See RELEASE-0.11.0.md for its build verification.
0.9.2 headset results are recorded below; 0.9.3 headset checks remain open.

Start with the [0.11.0 button fallback and calibration checks](RELEASE-0.11.0.md). No new headset results have been assumed.
...
- Install only one addon JAR, **0.11.0 on every client and server**. Single-player
