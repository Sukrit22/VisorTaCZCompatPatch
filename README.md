...
Current version: **0.6.5** · network protocol **10** · Java **17** · MIT license.
...
- Supported default TaCZ models: **Glock 17, M4A1, M870**.
...
- Optional Gun Durability integration: Glock/M4 stovepipe, double-feed, and dud handling states. M870 supports generic jam clearing through a completed pump cycle. No durability mod is required for normal operation.
...
**M870:** grab a shell preview at the cyan pouch, release at the green loading port, then pull the yellow pump grip back and forward. Compatible inventory ammunition is consumed only on a valid insertion. Hold the pump forward for support. Its blue calibration point is an aiming reference, not a separate grab zone.
...
| `/visor_tacz_test jam [stovepipe\|double_feed\|dud]` | Operator-only test with compatible Gun Durability; M870 only accepts generic/stovepipe test |
...
| --- | --- |
| TA31 ACOG | Reported black in VR with shaders off; unresolved. QMK-152 was reported working. |
...
| Other mechanisms | No direct M870 breech loading, bolt-release/safety gestures, or additional gun mechanisms beyond these three profiles. |
| Jams | Detailed three-type handling is Glock/M4 only and requires verified optional integration. Final frozen-action animations and dedicated sound cues need more work. Heat/cook-off simulation is not added. |
...
- Latest build: **61 unit tests passed**. Build success is not proof of in-headset correctness.
