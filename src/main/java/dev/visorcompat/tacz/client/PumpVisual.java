...
public final class PumpVisual {
    public static void render(PoseStack matrices,WeaponProfile profile,GunPose gun,VRPlayerPose pose,Handling.Phase phase,int light){
        ...
        var model=ammo.getAmmoEntityModel();var texture=ammo.getAmmoEntityTextureLocation();
        if(model==null||texture==null){model=ammo.getShellModel();texture=ammo.getShellTextureLocation();}
