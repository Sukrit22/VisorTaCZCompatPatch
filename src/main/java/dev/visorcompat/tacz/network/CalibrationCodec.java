...
public final class CalibrationCodec {
    public static void write(net.minecraft.network.FriendlyByteBuf b,Calibration c) {
        ...
        b.writeBoolean(c.scaleLock());b.writeFloat(c.gunScale());for(var box:c.zones().boxes()){b.writeFloat(box.width());b.writeFloat(box.height());b.writeFloat(box.depth());}
    }
    public static Calibration read(net.minecraft.network.FriendlyByteBuf b) {
        ...
        boolean locked=b.readBoolean();float scale=b.readFloat();var boxes=new dev.visorcompat.tacz.ZoneSizes.Box[7];
        for(int i=0;i<7;i++)boxes[i]=new dev.visorcompat.tacz.ZoneSizes.Box(b.readFloat(),b.readFloat(),b.readFloat());
        return new Calibration(v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8],
            new dev.visorcompat.tacz.InteractionOffsets(p[0],p[1],p[2],p[3],p[4],p[5],p[6]),scale,new dev.visorcompat.tacz.ZoneSizes(boxes[0],boxes[1],boxes[2],boxes[3],boxes[4],boxes[5],boxes[6]),locked);
    }
}
