...
public final class RemoteSync {
    ...
    public static void tick(ServerPlayer p) {
        ...
        var state=new CompatNetwork.RemoteState(p.getUUID(),Profiles.key(p.getMainHandItem()),active,
            CompatNetwork.physical(p),ServerPhysical.phase(p),ServerPhysical.pull(p),CompatNetwork.calibration(p),active && dev.visorcompat.tacz.physical.PouchAmmo.magazineLoaded(p,ServerPhysical.phase(p)),ServerPhysical.anchor(p));
